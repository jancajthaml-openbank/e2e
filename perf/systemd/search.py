...
class Search(Unit):
  ...
  def __init__(self):
    ...
    self.units.append('search-rest')

  def teardown(self):
    def eventual_teardown():
      try:
        out = subprocess.check_output(["journalctl", "-o", "short-precise", "-u", 'search-rest'], stderr=subprocess.STDOUT).decode("utf-8").strip()
  ...
  def restart(self) -> bool:
    def eventual_restart():
      try:
        subprocess.check_call(["systemctl", "restart", "search-rest"], stdout=Unit.FNULL, stderr=subprocess.STDOUT)
      except subprocess.CalledProcessError as ex:
        raise RuntimeError("Failed to restart search-rest with error {0}".format(ex))
  ...
  def reconfigure(self, params) -> None:
    ...
    if not self.restart():
      raise RuntimeError("search-rest failed to restart")
